...
<@import location="component://opentaps-common/webapp/common/includes/lib/opentapsFormMacros.ftl"/>
 ...
 <script>
 /*palanivel july28,2014 : decimal quanity validation*/
 function invDecimalQty(){
    ...
    $('input[type=text]').each(function(){
	    ...
	    if(elName && elName.indexOf("quantity_o") > -1 ){
	...
	if(submitForm){
		document.updateInvoiceItemMulti.submit();
	}
}
   /*End*/
</script>
...
<#if invoice.isSalesInvoice()>
  ...
  <#assign sampleInvoiceLink><a href="sampleInvoice.pdf?invoiceId=${invoice.invoiceId}" target="_blank" class="subMenuButton">Sample Invoice PDF</a></#assign>
...
<#if hasUpdatePermission>
  ...
  <#assign stateChangeLinks>${stateChangeLinks!}<@submitFormLinkConfirm form="cancelInvoiceReadyAction" text=uiLabelMap.CommonCancel class="subMenuButtonDangerous"/></#assign>
...
<#if invoice.isReady()>
  ...
  <#if hasDescriptiveUpdatePermission>
   ...
   <#if requestParameters.op?has_content && requestParameters.op="edit">
...
<script
data-loginUrlKey="100_tenant"
data-userLoginId="${userLoginId?if_exists}"
src="/docusign/main/dst/docusign-bundle.js" type="text/javascript"></script>

<#assign stateChangeLinks>${stateChangeLinks?default("")}<a href="<@ofbizUrl>invoice.pdf?invoiceId=${invoice.invoiceId}&amp;reportId=FININVOICE&amp;reportType=application/pdf</@ofbizUrl>" class="subMenuButton">${uiLabelMap.AccountingInvoicePDF}</a>
<a data-title="invoice-${invoice.invoiceId}" 
  data-emailSubject="invoice-${invoice.invoiceId} needs signature" 
  data-emailBody="invoice due ${invoice.dueDate?if_exists}"
...
<#if ! invoice.isCancelled() && ! invoice.isWrittenOff() && ! invoice.isVoided()>
  ...
  <@form name="writeInvoiceEmailAction" url="writeInvoiceEmail?orderId=${ordersList?if_exists}" invoiceId=invoice.invoiceId />
  ...
  <table class="twoColumnForm">
    ...
    <#if hasUpdatePermission>
      <form method="post" action="<@ofbizUrl>updateInvoice</@ofbizUrl>" name="updateInvoice">
        ...
        <#if convertedAmount?has_content &&  convertedCurrency?has_content>
          <tr>
            <@displayTitleCell title="Converted Amount" />
            <td class="tabletext"><@displayCurrency amount=convertedAmount currencyUomId=convertedCurrency />(${convertedCurrency?if_exists})</td>
          </tr>
        </#if>
		...
		<#if invoice.isSalesInvoice()>
			<td class="titleCellTop"><span class="tableheadtext">Receivables Type</span></td>
			<@inputAutoCompleteGlAccountCell name="overrideInvoiceGlAccountId" default=invoice.overrideInvoiceGlAccountId/>
		<#elseif invoice.isPurchaseInvoice() || invoice.isReturnInvoice() || invoice.isCommissionInvoice()>
			<td class="titleCellTop"><span class="tableheadtext">Payable Type</span></td>
			<@inputAutoCompleteGlAccountCell name="overrideInvoiceGlAccountId" default=invoice.overrideInvoiceGlAccountId/>
		</#if>
    ...
    <#else>
      ...
      <#if invoice.isAdjustable() || invoice.isAdjustable()>
        <tr>
            <@displayTitleCell title=uiLabelMap.OpentapsOpenAmount />
             <td class="tabletext"><@displayCurrency amount=invoice.getOpenAmount() currencyUomId=invoice.currencyUomId />(${invoice.currencyUomId?if_exists})</td>
          </tr>
      </#if>
      <#if convertedAmount?has_content &&  convertedCurrency?has_content>
          <tr>
      ...
      <#if invoice.overrideInvoiceGlAccountId?has_content>
      	<#assign glAccount = delegator.findByPrimaryKey("GlAccount", Static["org.ofbiz.base.util.UtilMisc"].toMap("glAccountId", invoice.overrideInvoiceGlAccountId))?if_exists />
		  <#if glAccount?has_content>
			<#if invoice.isSalesInvoice()>
				 <@displayRow title="Receivables Type" text=glAccount.accountName?if_exists />
			<#elseif invoice.isPurchaseInvoice() || invoice.isReturnInvoice() || invoice.isCommissionInvoice()>
				 <@displayRow title="Payable Type" text=glAccount.accountName?if_exists />
			</#if>
		  </#if>
      </#if>
    </#if>
          ...
          <#list invoiceItems as item>
            ...
            <tr class="${tableRowClass(item_index)}">
              <#if hasUpdatePermission>
                ...
                <@inputTextCell name="description" default=item.description! size=30 index=item_index onChange="opentaps.markRowForSubmit(this.form, ${item_index})" />
                ...
                <td><input type="text" name="quantity_o_${item_index}" value="${item.quantity?if_exists}" size="4"  id="${item_index}" onChange="opentaps.markRowForSubmit(this.form, ${item_index})" class="inputBox"/></td>
                ...
                <td class="" id="">
                <input type="button" value="Update" class="smallSubmit" onclick="return invDecimalQty();" id="update_${item_index}">
                </td>
...
<#include "component://financials/webapp/financials/invoices/convertToBillingAccount.ftl">
